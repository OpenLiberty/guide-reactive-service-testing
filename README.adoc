...
Navigate to the `start/kitchen` directory.

and run the following Maven goal to start Open Liberty in development mode:
...
however, a customer serializer has been provided for you, and is configured to the producer's
...
The [hotspot=testInitFoodOrder file=2]`testInitFoodOrder` test will be executed first, and will produce a message to the
...
Open Liberty development mode automatically recompiles and updates your test code changes. To run the tests, press the
