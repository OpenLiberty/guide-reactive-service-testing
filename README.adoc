...
The [hotspot=AppContainerConfig file=2]`AppContainerConfig` class externalizes test container setup and configuration, so you can use the same application containers across multiple tests.The [hotspot=container hotspot=container2 file=2]`@Container` annotation denotes an application container that is started up and used in the tests.

Two containers are used for testing the `system` service: the [hotspot=system file=2]`system` container, which you built, and the [hotspot=kafka file=2]`kafka` container, which receives messages from the `system` service.

The [hotspot=dependsOn file=2]`dependsOn()` method specifies that the `system` service container must wait until the `kafka` container is ready before it can start.
...
The test uses the [hotspot=KafkaConsumer hotspot=KafkaConsumer2 file=3]`KafkaConsumer` client API and is configured by using the [hotspot=KafkaConsumerClient file=3]`@KafkaConsumerClient` annotation. The consumer client is configured to consume messages from the [hotspot=systemLoadTopic file=3]`system.load` topic in the `kafka` container. To learn more about Kafka APIs and how to use them, check out the https://kafka.apache.org/documentation/#api[official Kafka Documentation^].

To consume messages from a stream, the messages need to be deserialized from bytes. Kafka has its own default deserializer, but a custom deserializer is provided for you. The deserializer is configured to the consumer’s [hotspot=valueDeserializer file=3]`valueDeserializer` and is implemented in the [hotspot=SystemLoadDeserializer file=4]`SystemLoad` class.

The running `system` service container produces messages to the [hotspot=systemLoad file=5]`systemLoad` Kafka topic, as denoted by the [hotspot=Outgoing file=5]`@Outgoing` annotation. The [hotspot=testCpuStatus file=3]`testCpuStatus()` test method [hotspot=poll file=3]`polls` a record from Kafka every 3 seconds until the timeout limit. It then [hotspot=assert file=3]`verifies` that the record polled matches the expected record.
