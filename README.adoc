...
To try out the application, go to the `finish` directory and run the following Maven goal to install the `models`
...
[role='no_copy']
```
 Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 33.001 s - in it.io.openliberty.guides.kitchen.KitchenEndpointIT
...
----
#Create the `KitchenEndpointIT` class.#
...
The [hotspot=testInProgress file=2]`testInProgress` test will be executed first, and will be producing a message to the
[hotspot=foodTopic file=2]`foodTopic` topic. The `kitchen` service will consume messages from that stream, as denoted
by the [hotspot=Incoming file=4]`@Incoming` annotation on the [hotspot=initFoodOrder file=4]`initFoodOrder()` method.
The same method will publish messages to the `foodOrderPublishStatus` stream as denoted by the
[hotspot=Outgoing file=4]`@Outgoing` annotation.
...
[role='no_copy']
```
 Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 25.674 s - in it.io.openliberty.guides.kitchen.KitchenEndpointIT
