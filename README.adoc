...
Create an integration test to assess the fundamental functionality of the System microservice. If a test failure occurs, then you might have introduced a bug into the code.
...
It's important to note that [hotspot=withListener file=0]`withListener()` has been configured to `kafka:19092`, as the containerized system service will function as an additional producer. Therefore, the Kafka container needs to set up a listener to accommodate this requirement. For further details on utilizing additional consumer or producer with Kafka container, please visit https://java.testcontainers.org/modules/kafka/[official Kafka Test Container Documentation^]

Since containers are isolated by default, facilitating communication between the `kafkaContainer` and the `systemContainer` requires placing them on the same [hotspot=network1 hotspot=network2 file=0]`network`. The [hotspot=dependsOn file=0]`dependsOn()` method is utilized to indicate that the `system` service container should commence only after ensuring the readiness of the kafka container. 

During the testing phase, covering both dev mode and Testcontainers integration tests, a method named [hotspot=isServiceRunning file=0]`isServiceRunning` has been introduced. This method serves the purpose of verifying the presence of a local container hosting the system service. 
...
Create an integration test to assess the fundamental functionality of the Inventory microservice.
