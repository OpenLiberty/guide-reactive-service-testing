...
Two containers are implemented for testing the `system` service: the `kafkaContainer` container is responsible for receiving messages from the system service hosted in the `systemContainer`. It's important to note that [hotspot=withListener file=0]`withListener()` has been configured to `kafka:19092`, as the containerized system service will function as an additional producer. Therefore, the Kafka container needs to set up a listener to accommodate this requirement. For further details on utilizing additional consumer or producer with Kafka container, please visit https://java.testcontainers.org/modules/kafka/[official Kafka Test Container Documentation^]
...
The test employs the [hotspot=KafkaConsumer file=0]`KafkaConsumer` client API, configuring the consumer client to use the [hotspot=BootstrapSetting1 file=0]`BOOTSTRAP_SERVERS_CONFIG` with the Kafka broker address if a local system service container is present. In the absence of a local service container, it utilizes the [hotspot=BootstrapSetting2 file=0]`getBootstrapServers()` method to obtain the broker address from the Kafka Testcontainer. Subsequently, the consumer is set up to consume messages from the [hotspot=systemLoadTopic file=0]`system.load` topic within the `Kafka` container.
