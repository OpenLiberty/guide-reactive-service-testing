...
To execute the system service with the `devc` goal, configure the container by specifying the options within [hotspot=devc_config file=0]`<containerRunOpts>`. The development mode container is configured to connect to the `reactive-app` Docker network.
...
The active `system` service actively seeks a Kafka topic for message push operations. Upon the successful initialization of a Kafka service, the `system` service establishes connectivity by utilizing the identical `bootstrap server` address. Additionally, the running `system` exposes its service on port `9083` for testing purposes in dev mode with container.
