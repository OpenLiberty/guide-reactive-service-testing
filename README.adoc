...
Initiate the services in dev mode by executing the following command to launch a Kafka instance replicating the production environment. The `startKafka` script will launch a local Kafka container and establish a `reactive-app` docker network that allows the `system` and `inventory` microservices to connect to the Kafka message broker.
...
To launch the `system` microservice in dev mode with container, configure the container by specifying the options within [hotspot=devc_config file=0]`<containerRunOpts>` configuration for connecting to the `reactive-app` Docker network and exposing the container port.
