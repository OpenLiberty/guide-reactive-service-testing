...
Initiate local testing by executing the following command to start a Kafka instance replicating the production environment. The `startKafka` script will launch a local Kafka container, enabling the `system/inventory` service to run in development mode with container goals. The script also establishes a `reactive-app` docker network, allowing the local `system/inventory` service to connect to the Kafka message broker:
[role='command']
```
...
To execute the system service with the `devc` goal, configure the container by specifying the options within [hotspot=devc_config file=0]`<containerRunOpts>`. The dev mode container is configured to connect to the `reactive-app` Docker network.
...
The active `system` service actively seeks a Kafka topic for message push operations. Upon the successful initialization of a Kafka service, the `system` service establishes connectivity to Kafka message broker by utilizing the `bootstrap server` address. Additionally, the running `system` exposes its service on port `9083` for testing purposes in dev mode with container.
...
Two containers are implemented for testing the `system` service: the `kafkaContainer` container is responsible for receiving messages from the system service hosted in the `systemContainer`. It's important to note that [hotspot=withListener file=0]`withListener()` has been configured to `kafka:19092`, as the system service will function as an additional producer. Therefore, the Kafka container needs to set up a listener to accommodate this requirement. For further details on utilizing additional consumer or producer with Kafka container, please visit https://java.testcontainers.org/modules/kafka/[official Kafka Test Container Documentation^]
...
The test utilizes the [hotspot=KafkaConsumer hotspot=KafkaConsumer2 file=0]`KafkaConsumer` client API, with the consumer client configured to consume messages from the [hotspot=systemLoadTopic file=0]`system.load` topic within the `kafka` container. 

If a local system service container is detected, the [hotspot=BootstrapSetting1 file=0]`BOOTSTRAP_SERVERS_CONFIG` will be configured to utilize the broker address of the Kafka service running within the local container. Alternatively, if a local service container is not detected, it will be configured to use the [hotspot=BootstrapSetting2 file=0]`getBootstrapServers()` method to acquire the broker address from the Kafka Testcontainer.
...
Similar to system service testing, the configuration of the producer client's `BOOTSTRAP_SERVERS_CONFIG` depends on whether a local inventory service container is detected.  In addition, the producer is configured with a custom serializer provided in the [hotspot=SystemLoadSerializer file=1]`SystemLoad` class.
