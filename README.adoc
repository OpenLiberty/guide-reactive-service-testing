...
Upon starting the container, the [hotspot=getSystems file=3]`getSystems` method declared in the `InventoryResourceClient` is invoked. To facilitate this, a `ResteasyClient` has been introduced to construct a REST client, enabling HTTP requests to be made to the running application.
