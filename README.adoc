...
With Open Liberty devc mode, you can use Testcontainers to run tests on an operational Open Liberty server. Navigate to the `start/system` directory.
...
To start the test with a running local service, execute the following command to start the local Kafka instance, replicating the same Kafka environment setup as production:
[role='command']
```
./script/startKafka.sh
```
...
If you aren't running in devc mode, you can run the tests by running the following command:
...
As you did with the `system` service, run Open Liberty in devc mode to listen for file changes:
...
Similar to system service testing, the configuration of the producer client's `BOOTSTRAP_SERVERS_CONFIG` depends on whether a local inventory service container is detected. In this scenario, it will utilize the [hotspot=BootstrapServerConfig file=0]`BOOTSTRAP_SERVERS_CONFIG` exported from the local inventory service container or the [hotspot=BootstrapServerConfig2 file=0]`BOOTSTRAP_SERVERS_CONFIG` of the kafkaContainer. In addition, the producer is configured with a custom serializer provided in the [hotspot=SystemLoadSerializer file=1]`SystemLoad` class.
...
When you're finished trying out the microservice, you can stop the local Kafka container by running the following command:
[role='command']
```
./script/stopKafka.sh
```
