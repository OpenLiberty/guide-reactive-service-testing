...
`src/test/java/it/io/openliberty/guides/system/AppContainerConfig.java`
----
...
is configured by using the [hotspot=KafkaConsumerConfig file=2]`@KafkaConsumerConfig` annotation. The consumer client is configured
...
[hotspot=KafkaProducerConfig file=0]`@KafkaProducerConfig` annotation configures the producer to use the custom serializer provided in
the [hotspot=SystemLoadSerializer file=1]`SystemLoad` class. The [hotspot=KafkaProducerConfig file=0]`@KafkaProducerConfig` annotation
...
In this example, it is configured to produce messages to the [hotspot=systemLoadTopic file=0]`systemLoadTopic` topic.
