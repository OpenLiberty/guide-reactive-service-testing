...
The [hotspot=AppContainerConfig file=1]`AppContainerConfig` class externalizes test container setup and configuration, so you can use the same application containers across multiple tests.The [hotspot=container hotspot=container2 file=1]`@Container` annotation denotes an application container that is started up and used in the tests.

Two containers are used for testing the `system` service: the [hotspot=system file=1]`system` container, which you built, and the [hotspot=kafka file=1]`kafka` container, which receives messages from the `system` service.

The [hotspot=dependsOn file=1]`dependsOn()` method specifies that the `system` service container must wait until the `kafka` container is ready before it can start.
...
The test uses the [hotspot=KafkaConsumer hotspot=KafkaConsumer2 file=2]`KafkaConsumer` client API and is configured by using the [hotspot=KafkaConsumerClient file=2]`@KafkaConsumerClient` annotation. The consumer client is configured to consume messages from the [hotspot=systemLoadTopic file=2]`system.load` topic in the `kafka` container. To learn more about Kafka APIs and how to use them, check out the https://kafka.apache.org/documentation/#api[official Kafka Documentation^].

To consume messages from a stream, the messages need to be deserialized from bytes. Kafka has its own default deserializer, but a custom deserializer is provided for you. The deserializer is configured to the consumer’s [hotspot=valueDeserializer file=2]`valueDeserializer` and is implemented in the [hotspot=SystemLoadDeserializer file=3]`SystemLoad` class.

The running `system` service container produces messages to the [hotspot=systemLoad file=4]`systemLoad` Kafka topic, as denoted by the [hotspot=Outgoing file=4]`@Outgoing` annotation. The [hotspot=testCpuStatus file=2]`testCpuStatus()` test method [hotspot=poll file=2]`polls` a record from Kafka every 3 seconds until the timeout limit. It then [hotspot=assert file=2]`verifies` that the record polled matches the expected record.
